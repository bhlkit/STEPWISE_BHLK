...
class SplashScreen(customtkinter.CTk):
    def __init__(self):
        ...
        self.configure(fg_color=config.COLOR_WHITE) 

        current_dir = os.path.dirname(os.path.abspath(__file__))
        image_path = os.path.join(current_dir,"assets", "splash_screen.png") 
        ...
        try:
            ...
            self.lbl_img = customtkinter.CTkLabel(self, text="Splash Image Not Found", font=config.FONT_BOLD)
...
class StepwiseApp(customtkinter.CTk):
    def __init__(self):
        ...
        self.configure(fg_color=config.COLOR_BG_MAIN)

        current_dir = os.path.dirname(os.path.abspath(__file__))
        icon_path = os.path.join(current_dir,"assets", "bhlk_logo.ico") 
        ...
        # 1. KONTROL FRAME
        self.ctrl_frame = customtkinter.CTkFrame(self, fg_color=config.COLOR_WHITE, corner_radius=10, border_width=1, border_color="#e0e0e0")
        self.ctrl_frame.grid(row=0, column=0, padx=20, pady=(20, 10), sticky="ew")
        
        self.btn_file = customtkinter.CTkButton(self.ctrl_frame, text="📂 1. Pilih File Data", command=self.browse_file, font=config.FONT_BOLD, fg_color=config.COLOR_SECONDARY, text_color="white", hover_color="#15224a", corner_radius=8, height=35)
        self.btn_file.grid(row=0, column=0, padx=15, pady=15)
        
        self.lbl_file = customtkinter.CTkLabel(self.ctrl_frame, text="File belum dipilih...", text_color="#636e72", font=config.FONT_MAIN)
        self.lbl_file.grid(row=0, column=1, padx=10, pady=15, sticky="w")

        self.btn_run = customtkinter.CTkButton(self.ctrl_frame, text="▶ JALANKAN ANALISIS", command=self.run_analysis, font=config.FONT_BOLD, fg_color=config.COLOR_PRIMARY, text_color=config.COLOR_SECONDARY, hover_color="#e6a700", corner_radius=8, height=35, width=180)
        self.btn_run.grid(row=0, column=2, padx=15, pady=15)
        
        self.btn_save = customtkinter.CTkButton(self.ctrl_frame, text="💾 SIMPAN EXCEL", command=self.save_to_excel, font=config.FONT_BOLD, fg_color="transparent", border_width=2, border_color=config.COLOR_SECONDARY, text_color=config.COLOR_SECONDARY, hover_color="#eef2ff", corner_radius=8, height=35, state="disabled")
        self.btn_save.grid(row=0, column=3, padx=15, pady=15)

        self.ctrl_frame.grid_columnconfigure(4, weight=1)

        # 2. HASIL TEXT AREA
        self.res_frame = customtkinter.CTkFrame(self, fg_color=config.COLOR_WHITE, corner_radius=10, border_width=1, border_color="#e0e0e0")
        ...
        self.txt_out = customtkinter.CTkTextbox(self.res_frame, font=config.FONT_MONO, text_color=config.COLOR_SECONDARY, fg_color="#fafafa", corner_radius=5)
        ...
        self.tab_view = customtkinter.CTkTabview(self.plot_container, height=300, fg_color=config.COLOR_WHITE, segmented_button_fg_color="#e0e0e0", segmented_button_selected_color=config.COLOR_PRIMARY, segmented_button_selected_hover_color="#e6a700", segmented_button_unselected_color="#e0e0e0", segmented_button_unselected_hover_color="#d1d1d1", text_color=config.COLOR_SECONDARY)
    ...
    def browse_file(self):
        ...
        if f:
            self.file_path = f
            self.lbl_file.configure(text=f"File: {os.path.basename(f)}", text_color=config.COLOR_SECONDARY)
    ...
    def run_analysis(self):
        ...
        try:
            ...
            # Panggil fungsi dari module logic.py
            df = logic.read_data_native(self.file_path)
            if df is None: return
            
            if len(df.columns) < 2:
                messagebox.showerror("Error", "Data harus memiliki minimal 2 kolom.")
                return
            ...
            out += f" Fin Limit: {fin} (Locked - High Sensitivity)\n"
            out += "===========================================================\n\n"
            
            out += "--- 1. DESCRIPTIVE STATISTICS ---\n"
            out += logic.get_descriptive_stats(df) + "\n\n"
            out += "--- 2. CORRELATION MATRIX ---\n"
            out += logic.get_correlation_matrix(df) + "\n\n"

            # Panggil fungsi stepwise dari logic.py
            results = logic.stepwise_regression_final(df_proc, target_var, fin_threshold=fin)
            ...
            self.final_equation = logic.generate_equation(final_model, target_var, use_log)
    ...
    def plot_hydrograph(self, y_act, y_pred):
        ...
        try:
            fig, ax = plt.subplots(figsize=(10, 3.5), dpi=100, facecolor=config.COLOR_WHITE)
            ax.set_facecolor(config.COLOR_WHITE)
            
            ax.plot(y_act.values.flatten(), label='Observasi', color=config.COLOR_SECONDARY, linewidth=1.5)
            ax.plot(y_pred.values.flatten(), label='Prediksi', color=config.COLOR_ACCENT_RED, linestyle='--', linewidth=1.5)
            
            ax.set_title("Hydrograph Comparison", color=config.COLOR_SECONDARY, fontsize=10, fontweight='bold')
            ax.set_ylabel("Debit", color=config.COLOR_SECONDARY, fontsize=9)
            ax.tick_params(colors=config.COLOR_SECONDARY, labelsize=8)
            
            legend = ax.legend(fontsize=8)
            plt.setp(legend.get_texts(), color=config.COLOR_SECONDARY)
            ax.grid(True, alpha=0.2, color=config.COLOR_SECONDARY)
            
            for spine in ax.spines.values():
                spine.set_edgecolor(config.COLOR_SECONDARY)
                spine.set_alpha(0.5)
    ...
    def plot_correlation_graph(self):
        ...
        try:
            ...
            ax.plot(x_pos, y_values, color=config.COLOR_SECONDARY, linewidth=2, zorder=1)
            ax.scatter(x_pos, y_values, color=config.COLOR_PRIMARY, s=60, zorder=2, edgecolors=config.COLOR_SECONDARY)

            ax.set_xticks(x_pos)
            ax.set_xticklabels(x_labels, rotation=45, ha='right', fontsize=9)
            ax.set_title(f"Grafik Korelasi (r) Tiap Stasiun terhadap {self.stored_target}", color=config.COLOR_SECONDARY, fontsize=10, fontweight='bold')
            ax.set_ylabel("Nilai Korelasi (r)", color=config.COLOR_SECONDARY, fontsize=9)
            ax.tick_params(colors=config.COLOR_SECONDARY, labelsize=8)
            ax.grid(True, linestyle='--', alpha=0.2, color=config.COLOR_SECONDARY)
            ...
            for i, v in enumerate(y_values):
                ax.text(i, v + 0.02, f"{v:.2f}", ha='center', fontsize=8, color=config.COLOR_SECONDARY)
            ...
            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                ...
                if self.corr_data_sorted is not None:
                    try:
                        ...
                        plt.title(f"Grafik Korelasi (r) Tiap Stasiun terhadap {self.stored_target}", color=config.COLOR_SECONDARY, fontsize=10, fontweight='bold')
                        ...
                        # Simpan ke Sheet Matriks Korelasi
